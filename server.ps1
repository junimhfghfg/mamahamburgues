...
Write-Host "   http://localhost:$port/Site Htmls/Tela1.html" -ForegroundColor Cyan
  ...
  while ($true) {
    ...
    try {
      ...
      if ($nread -gt 0) {
        ...
        if ($local -eq '' -or $local -eq '/') { $local = 'Site Htmls/index.html' }
        ...
        if (Test-Path $filePath -PathType Leaf) {
          ...
          $body = [System.Text.Encoding]::UTF8.GetBytes("<!DOCTYPE html><html><body><h2>404 - Nao encontrado</h2><p>$local</p><a href='/Site Htmls/Tela1.html'>Ir para Tela1</a></body></html>")
